...
def ap_per_class(tp, conf, pred_cls, target_cls, v5_metric=False, plot=False, save_dir='.', names=(), eps=1e-16, prefix=''):
    ...
    def plot_auroc_curve(self, fpr_, tpr_, auc_scores, save_dir='', names=()):
        ...
        if 0 < len(names) < 21:  # display per-class legend if < 21 classes
            for i in range(len(names)):
                ax.plot(fpr_[i], tpr_[i], linewidth=3, label=f'{names[i]} {auc_scores[i]:.3f}')  # plot(F_PR, T_PR)
        else:
            for i in range(len(names)):
                ax.plot(fpr_[i], tpr_[i], linewidth=3, color='grey')  # plot(F_PR, T_PR)

        ax.plot([0, 1], [0, 1], linestyle='--', color='black', linewidth=1)  # diagonal line
        #mean_tpr = np.mean(tpr_, axis=0)
        #mean_fpr = np.mean(fpr_, axis=0)
        #ax.plot(fpr_.mean(1), tpr_.mean(1), color='black', linewidth=3, label=auc_scores.mean())
